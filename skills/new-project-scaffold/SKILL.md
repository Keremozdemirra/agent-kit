...
description: Creates a new numbered project in the projects/NNN-name/ layout used by esg-toolkit, analyst-toolkit, open-climate-data and unitguard — package directory, unittest suite, executable example, pyproject with requires-python >=3.9 and zero dependencies, and a README whose "what this is not" section is mandatory. Also updates BACKLOG.md and the root README project table, the two places people forget. Use to start a new project or tool in those repos, to pick up the next unchecked BACKLOG item — "yeni proje", "yeni arac ekle", "scaffold a project" — or to check whether an existing project follows the layout. Not for orchestrating a whole multi-part build; use proje.
---
