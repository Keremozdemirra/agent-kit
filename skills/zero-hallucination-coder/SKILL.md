...
Capture what is actually being built before any planning happens. Ask and fully resolve:
...
Break the task into atomic stories — small enough that each fits in one response.
...
**Step D — Handoff note:** what was built (one sentence), how to test (exact steps, not "it should work"), what to watch for (edge cases or fragile assumptions), next story.
...
Prevents "context rot" — silent quality degradation as the context window fills.
...
- **Just execute:** typo, reformat, lint, docstring.
